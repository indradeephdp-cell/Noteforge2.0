...
// Convert SVG element to PNG data URL — large and clear
function svgToDataURL(svgEl){
  return new Promise((resolve)=>{
    try{
      const clone=svgEl.cloneNode(true);
      // Force ALL text to be larger and readable
      clone.querySelectorAll("text").forEach(t=>{
        t.style.fontSize="16px";
        t.style.fontFamily="Arial, sans-serif";
      });
      clone.querySelectorAll(".label, .nodeLabel, foreignObject, span, div").forEach(el=>{
        el.style.fontSize="15px";
        el.style.fontFamily="Arial, sans-serif";
      });
      // Use full A4 width: 480pt usable = 640px at 96dpi
      const targetW=700;
      const vb=svgEl.getAttribute("viewBox");
      let vbW=svgEl.width?.baseVal?.value||svgEl.getBoundingClientRect().width||500;
      let vbH=svgEl.height?.baseVal?.value||svgEl.getBoundingClientRect().height||300;
      if(vb){const p=vb.split(/[\\s,]+/).map(Number);if(p.length>=4){vbW=p[2];vbH=p[3];}}
      // Scale to fill target width fully
      const scale=targetW/Math.max(vbW,1);
      const dispW=targetW;
      const dispH=Math.round(vbH*scale);
      clone.setAttribute("width",dispW);
      clone.setAttribute("height",dispH);
      clone.setAttribute("viewBox",\`0 0 \${vbW} \${vbH}\`);
      clone.style.cssText=\`background:#f8f5eb;\`;
      ...
      img.onload=()=>{
        ...
        const px=2;
        canvas.width=dispW*px;canvas.height=dispH*px;
        const ctx=canvas.getContext("2d");
        ctx.fillStyle="#f8f5eb";ctx.fillRect(0,0,canvas.width,canvas.height);
        ctx.scale(px,px);
        ctx.drawImage(img,0,0,dispW,dispH);
        resolve({dataUrl:canvas.toDataURL("image/png"),w:dispW,h:dispH});
      };
      img.onerror=()=>resolve(null);
      img.src=dataUrl;
    }catch(e){console.warn("SVG capture error:",e);resolve(null);}
  });
}
...
async function dlPdf(){
  ...
  // Pre-capture all rendered diagrams as PNG images
  const diagramImages=[];
  const mmdBoxes=document.querySelectorAll(".mmd-box[data-done='1']");
  for(const box of mmdBoxes){
    const svgEl=box.querySelector("svg");
    if(svgEl){
      const result=await svgToDataURL(svgEl);
      diagramImages.push(result);
    }else{
  ...
  try{
    ...
    // Render text with inline bold/italic formatting in gold color
    function drawRichText(doc,text,x,startY,maxW,fontSize,baseColor,lineH){
      const cleanText=text.replace(/[^\\x00-\\x7F]/g,"");
      // Split by bold markers
      const parts=cleanText.split(/(\\*\\*[^*]+\\*\\*)/g);
      if(parts.length<=1){
        // No bold — render normally
        doc.setFontSize(fontSize);doc.setFont("helvetica","normal");doc.setTextColor(...baseColor);
        const wr=doc.splitTextToSize(cleanText,maxW);
        wr.forEach((line,idx)=>{doc.text(line,x,startY+idx*lineH);});
        return wr.length*lineH;
      }
      // Has bold parts — render word by word inline
      let cx=x;let cy=startY;const rightEdge=x+maxW;
      parts.forEach(part=>{
        const isBold=part.startsWith("**")&&part.endsWith("**");
        const word=(isBold?part.slice(2,-2):part).replace(/[^\\x00-\\x7F]/g,"");
        if(!word)return;
        doc.setFontSize(fontSize);
        if(isBold){doc.setFont("helvetica","bold");doc.setTextColor(0,100,50);}
        else{doc.setFont("helvetica","normal");doc.setTextColor(...baseColor);}
        // Split into words and render
        const words=word.split(/\\s+/).filter(w=>w);
        words.forEach((w,wi)=>{
          const ww=doc.getTextWidth(w+" ");
          if(cx+ww>rightEdge&&cx>x){cx=x;cy+=lineH;}
          doc.text(w,cx,cy);cx+=ww;
        });
      });
      return cy-startY+lineH;
    }
    ...
    for(let i=0;i<lines.length;i++){
      ...
      if(t.startsWith("\`\`\`mermaid")){
        ...
        while(i<lines.length&&!lines[i].trim().startsWith("\`\`\`")){code+=lines[i]+"\\n";i++;}
        code=code.trimEnd();
        const titleM=code.match(/%%\\s*(.+)/);
        const dTitle=titleM?cl(titleM[1]):"Diagram";
        const typeM=code.match(/^(flowchart|graph|timeline|sequenceDiagram|pie|gantt)/m);
        const dType=typeM?typeM[1]:"graph";
        const imgData=diagramImages[diagramIdx]||null;
        diagramIdx++;
        if(imgData){
          // Use FULL page width for diagram — max height 320pt
          const aspect=imgData.h/Math.max(imgData.w,1);
          const dispW=TW;
          const dispH=Math.min(Math.round(dispW*aspect),320);
          chk(dispH+44);y+=8;
          // Cream box with navy border
          doc.setFillColor(248,244,232);doc.rect(ML-4,y-4,TW+8,dispH+32,"F");
          doc.setFillColor(...navy);doc.rect(ML-4,y-4,4,dispH+32,"F");
          // Gold header bar
          doc.setFillColor(...goldline);doc.rect(ML,y-4,TW+4,20,"F");
          doc.setFontSize(9);doc.setFont("helvetica","bold");doc.setTextColor(255,255,255);
          doc.text(dType.charAt(0).toUpperCase()+dType.slice(1)+": "+dTitle.slice(0,55),ML+7,y+9);
          // Embed diagram image at FULL width
          doc.addImage(imgData.dataUrl,"PNG",ML,y+20,dispW,dispH);
          doc.setDrawColor(...tblbord);doc.setLineWidth(0.5);doc.rect(ML-4,y-4,TW+8,dispH+32,"S");
          y+=dispH+44;
        }else{
          // Fallback text if image capture failed
          chk(40);y+=8;
          doc.setFillColor(248,244,232);doc.rect(ML-4,y-4,TW+8,35,"F");
          doc.setFillColor(...goldline);doc.rect(ML,y-4,TW+4,20,"F");
          doc.setFontSize(9);doc.setFont("helvetica","bold");doc.setTextColor(255,255,255);
          doc.text("["+dType+"] "+dTitle,ML+7,y+9);
          doc.setFontSize(9);doc.setTextColor(...body);doc.setFont("helvetica","italic");
          doc.text("(See preview for diagram)",ML+7,y+26);
          y+=48;
        }
        continue;
      }
      ...
      if(t.match(/^[-*+] /)){
        const raw=t.replace(/^[-*+] /,"").replace(/\`/g,"").replace(/[^\\x00-\\x7F*]/g,"").trim();
        const plainH=Math.ceil(doc.splitTextToSize(cl(raw),TW-16).length)*14+4;
        chk(plainH);
        doc.setFillColor(...navy);doc.circle(ML+4,y-3,2.2,"F");
        const h=drawRichText(doc,raw,ML+12,y,TW-16,10.5,body,14);
        y+=h+1;continue;
      }
      if(t.match(/^\\d+[.)]\\s/)){
        const num=t.match(/^(\\d+)/)?.[1]||"1";
        const raw=t.replace(/^\\d+[.)]\\s/,"").replace(/\`/g,"").replace(/[^\\x00-\\x7F*]/g,"").trim();
        const plainH=Math.ceil(doc.splitTextToSize(cl(raw),TW-20).length)*14+4;
        chk(plainH);
        doc.setFillColor(...navy);doc.circle(ML+7,y-3.5,7,"F");
        doc.setFontSize(7.5);doc.setFont("helvetica","bold");doc.setTextColor(255,255,255);
        doc.text(num,ML+7,y-1,{align:"center"});
        const h=drawRichText(doc,raw,ML+18,y,TW-20,10.5,body,14);
        y+=h+1;continue;
      }
      ...
      if(!t){y+=5;continue;}
      const raw2=t.replace(/\`/g,"").replace(/[^\\x00-\\x7F*]/g,"").trim();
      if(!raw2)continue;
      const plainH2=Math.ceil(doc.splitTextToSize(cl(t),TW).length)*14+4;
      chk(plainH2);
      const h2=drawRichText(doc,raw2,ML,y,TW,10.5,body,14);
      y+=h2+2;
    }
...
app.post("/api/generate", async (req, res) => {
  ...
  // Text input — single call
  if (!pdfBase64) {
    try {
      const parts = [{ text: "Create comprehensive exam-ready notes.\n\n" + SYS() + "\n\nCONTENT:\n" + prompt }];
      const r = await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=" + key, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ contents: [{ role: "user", parts }], generationConfig: { temperature: 0.4, maxOutputTokens: 65536, topP: 0.95 } })
      });
  ...
  // PDF input — auto chunk every 25 pages for complete coverage
  const from = parseInt(pageFrom) || 1;
  const to   = parseInt(pageTo)   || 33;
  const CHUNK = 25;
  const allNotes = [];
  ...
  for (let start = from; start <= to; start += CHUNK) {
    ...
    const parts = [
      ...
      { text: `Create comprehensive exam-ready notes from pages ${start} to ${end} of this PDF.\n\n${SYS()}` }
    ];
    try {
      const r = await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=" + key, {
        method: "POST", headers: { "Content-Type": "application/json" },
      ...
      const d = await r.json();
      if (d.error) return res.status(400).json({ error: `Pages ${start}-${end}: ` + d.error.message });
      const chunk = d?.candidates?.[0]?.content?.parts?.[0]?.text || "";
      if (chunk) allNotes.push(`\n\n---\n\n# Section: Pages ${start} to ${end}\n\n` + chunk);
    } catch(e) {
      return res.status(500).json({ error: `Failed on pages ${start}-${end}: ` + e.message });
    }
  }

  res.json({ text: allNotes.join("\n\n") });
});
app.get("*", (_, res) => { res.setHeader("Content-Type", "text/html; charset=utf-8"); res.send(HTML); });
app.listen(PORT, () => console.log("\n NoteForge AI at http://localhost:" + PORT + "\n"));
function SYS() {
  return "You are a world-class academic note synthesizer.\n\nSTART with:\nNote Style: [Name] - [reason]\nStyles: Hierarchical Outline / Cornell Method / Thematic Clustering / Data-Centric Analytics / Comparative Framework / Timeline + Analysis\n\nNOTES:\n- # ## ### #### headings\n- **Bold** key terms, dates, figures\n- > Blockquote for critical facts\n- --- between sections\n\nMANDATORY DIAGRAMS - ALL relevant types throughout notes:\n\nFlowchart:\n```mermaid\nflowchart TD\n  %% Title\n  A[\"Start\"] --> B{\"Decision\"}\n  B -->|\"Yes\"| C[\"Action A\"]\n  B -->|\"No\"| D[\"Action B\"]\n  C --> E[\"Result\"]\n```\n\nClassification:\n```mermaid\ngraph TD\n  %% Title\n  A[\"Main Concept\"] --> B[\"Category 1\"]\n  A --> C[\"Category 2\"]\n  B --> D[\"Sub Type 1\"]\n  B --> E[\"Sub Type 2\"]\n```\n\nTimeline - NO colons in text:\n```mermaid\ntimeline\n  title Title\n  1947 : India Independence\n  2007 : RKVY Started\n  2015 : Funding Revised\n```\n\nMind Map:\n```mermaid\ngraph LR\n  %% Title\n  A[\"Center\"] --> B[\"Branch 1\"]\n  A --> C[\"Branch 2\"]\n  B --> D[\"Detail\"]\n```\n\nMERMAID RULES: ALL labels in double quotes. NO <br/> tags. Max 5 words.\n\nTABLES for comparative data.\nCHART INSIGHTS for every chart in source.\n> Key Takeaway after each section.\n## Quick Revision - Top 10 Points\n## Exam Questions with Answers\n\nOUTPUT: Markdown only. Start with Note Style line.";
}
