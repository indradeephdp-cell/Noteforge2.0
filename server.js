...
.uz-icon{font-size:44px;line-height:1;margin-bottom:10px}
.uz-title{font-size:14px;font-weight:700;color:var(--text);margin-bottom:5px}
.uz-sub{font-size:11px;color:var(--text2)}
.uz-chip{display:inline-block;margin-top:9px;padding:4px 12px;border-radius:20px;border:1px solid var(--border);color:var(--text3);font-size:10px}
/* Visible styled file input button */
.file-input-btn{
  display:block;width:100%;margin-top:12px;
  padding:13px 16px;
  background:linear-gradient(135deg,#5c2a00,var(--gold));
  border:none;border-radius:11px;
  color:#07090f;font-family:'Source Sans 3',sans-serif;
  font-size:15px;font-weight:700;
  cursor:pointer;text-align:center;
  letter-spacing:.3px;
}
input[type=file].file-input-btn{
  color:#07090f;
}
input[type=file].file-input-btn::file-selector-button{
  display:none;
}
input[type=file].file-input-btn::-webkit-file-upload-button{
  display:none;
}
      ...
      <div id="pdf-sec">
        ...
        <div class="upload-zone" id="uz"
          ...
          <div id="uz-inner">
            ...
            <div class="uz-title" id="uz-title">Tap button below to select PDF</div>
            <div class="uz-sub">or drag &amp; drop your file here</div>
          </div>
        </div>

        <!-- MAIN UPLOAD BUTTON — fully visible native input -->
        <label style="display:block;width:100%;margin-top:10px;padding:14px;background:linear-gradient(135deg,#5c2a00,var(--gold));border-radius:11px;text-align:center;cursor:pointer;font-family:'Source Sans 3',sans-serif;font-size:15px;font-weight:700;color:#07090f;letter-spacing:.3px;box-sizing:border-box">
          📂 &nbsp; Tap Here to Select PDF File
          <input type="file" accept=".pdf,application/pdf" style="position:absolute;width:1px;height:1px;opacity:0;overflow:hidden;z-index:-1" onchange="onFileChange(event)"/>
        </label>
...
function processFile(file) {
  ...
  reader.onload = () => {
    ...
    document.getElementById("uz-inner").innerHTML=\`
      ...
      <div class="uz-sub">\${(file.size/1024/1024).toFixed(2)} MB · Tap button to replace</div>\`;
