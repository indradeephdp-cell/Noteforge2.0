...
function svgToDataURL(svgEl){
  return new Promise((resolve)=>{
    try{
      ...
      clone.style.background="#f8f5eb";
      // Get dimensions
      const vb=clone.getAttribute("viewBox");
      let w=480,h=300;
      if(vb){const p=vb.split(/\\s+/).map(Number);w=Math.min(p[2]||480,800);h=Math.min(p[3]||300,500);}
  ...
  try{
    ...
    for(let i=0;i<lines.length;i++){
      ...
      if(t.startsWith("\`\`\`mermaid")){
        ...
        diagramIdx++;
        if(imgData){
          // Calculate display size — fit to page width, max height 280pt
          const aspect=imgData.h/imgData.w;
          const dispW=TW;
          const dispH=Math.min(dispW*aspect,280);
          chk(dispH+44);y+=8;
          ...
          // Embed actual diagram image
          doc.addImage(imgData.dataUrl,"PNG",ML,y+20,dispW,dispH-8);
