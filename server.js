...
// ── Embedded HTML — no file path issues ──
...
.mmd-box{background:var(--bg2);border:2px solid var(--border);border-radius:14px;padding:28px 20px;margin:28px 0;overflow-x:auto;text-align:center;width:100%;page-break-inside:avoid;min-height:200px}
.mmd-box svg{display:block;margin:0 auto}
.mmd-box text{font-size:14px!important;font-family:'Source Sans 3',sans-serif!important}
.mmd-box .label div{font-size:14px!important;line-height:1.4!important}
.mmd-box .nodeLabel{font-size:14px!important}
.mmd-box .edgeLabel{font-size:13px!important;background:var(--bg2)!important;padding:2px 6px!important}
...
async function renderMermaid() {
  ...
  for (const el of els) {
    ...
    try {
      ...
      // Create a wrapper and inject SVG
      const wrapper = document.createElement("div");
      wrapper.style.cssText = "width:100%;overflow-x:auto;padding:10px 0";
      wrapper.innerHTML = svg;

      // Get the SVG element and make it large
      ...
      if (svgEl) {
        ...
        if (vb) {
          ...
          // Scale up by 2x minimum, fill container width
          const containerW = el.offsetWidth || 600;
          const scale = Math.max(containerW / vbW, 1.8);
          const newW = Math.round(vbW * scale);
          const newH = Math.round(vbH * scale);
          svgEl.setAttribute("width", newW + "px");
          svgEl.setAttribute("height", newH + "px");
          svgEl.style.cssText = \`width:\${newW}px;height:\${newH}px;min-width:\${newW}px;display:block;margin:0 auto\`;
        } else {
          // No viewBox — just make it big
          svgEl.setAttribute("width", "100%");
          svgEl.style.cssText = "width:100%;min-width:500px;display:block;margin:0 auto";
        }

        // Force ALL text in diagram to be larger
        svgEl.querySelectorAll("text, .label, .nodeLabel, .edgeLabel").forEach(t => {
          t.style.fontSize = "14px";
          t.style.fontFamily = "Georgia, serif";
        });
        svgEl.querySelectorAll(".node rect, .node circle, .node polygon").forEach(n => {
      ...
      // Store SVG for PDF
      if (svgEl) el.setAttribute("data-rendered","1");

    } catch(e) {
      el.innerHTML = \`
        <div style="background:#0f1521;border:1px solid #2a3550;border-radius:8px;padding:20px;text-align:left">
          <div style="color:#e8a020;font-size:12px;margin-bottom:10px;font-weight:700">📊 DIAGRAM — Syntax Error</div>
          <pre style="color:#8a96ac;font-size:12px;white-space:pre-wrap;line-height:1.7;overflow-x:auto">\${decodeURIComponent(el.getAttribute("data-code")||"")}</pre>
        </div>\`;
    }
  ...
  try {
    ...
    for(let i=0;i<lines.length;i++){
      ...
      if(t.startsWith("\`\`\`mermaid")){
        ...
        const boxH = Math.min(diagramLines.length * 16 + 80, 300);
        chk(boxH + 40);
        y += 8;

        // Draw diagram box — cream background
        doc.setFillColor(248, 245, 235);
        doc.rect(ML-4, y-4, TW+8, boxH+30, "F");

        // Navy left border
        doc.setFillColor(...C.navy);
        doc.rect(ML-4, y-4, 4, boxH+30, "F");
        ...
        doc.text(\`📊 \${typeLabel}: \${diagramTitle}\`, ML+8, y+11);

        // Draw diagram content lines
        let dy = y + 30;
        doc.setFontSize(9);
        doc.setFont("helvetica","normal");
        doc.setTextColor(...C.navy);

        // Draw simple visual representation based on diagram type
        if (diagramType === "timeline") {
          ...
          doc.setDrawColor(...C.goldline);
          doc.setLineWidth(2);
          doc.line(timeX, lineY, ML + TW - 10, lineY);

          events.slice(0, 8).forEach((ev, idx) => {
            const parts = ev.split(":").map(s=>s.trim());
            const year = parts[0] || "";
            const desc = parts[1] || "";
            const xPos = timeX + (idx * (TW-20) / Math.max(events.length-1, 1));
            doc.setFillColor(...C.navy);
            doc.circle(xPos, lineY, 4, "F");
            doc.setFontSize(8);
            doc.setTextColor(...C.navy);
            doc.text(year, xPos-10, lineY+14);
            const wrDesc = doc.splitTextToSize(desc, 60);
            doc.text(wrDesc[0]||"", xPos-10, lineY+24, {maxWidth:65});
          });
          dy = lineY + 45;

        } else {
          // For flowcharts, graphs, mind maps — show as structured text
          diagramLines.slice(0, 15).forEach(line => {
            const clean = line
              .replace(/["'\\[\\]{}()]/g, "")
              .replace(/-->/g, " → ")
              .replace(/---/g, " — ")
              .replace(/\\|[^|]*\\|/g, "")
              .replace(/subgraph/g, "  GROUP:")
              .replace(/end\\s*$/i, "")
              .trim();
            if (!clean) return;
            const indent = (line.match(/^\\s+/) || [""])[0].length;
            const xOffset = ML + 8 + Math.min(indent * 4, 60);
            if (dy + 14 > y + boxH + 20) return;
            doc.setFontSize(9.5);
            ...
            // Small bullet
            doc.setFillColor(...C.goldline);
            doc.circle(xOffset - 3, dy - 3, 1.5, "F");
            const wr = doc.splitTextToSize(clean, TW - indent*4 - 20);
            doc.text(wr[0], xOffset + 4, dy);
            dy += 14;
          });
        }
        ...
        doc.rect(ML-4, y-4, TW+8, boxH+30, "S");
...
Analyze content deeply and choose the best style:
...
Place diagrams RIGHT AFTER the section they explain. Minimum 2 diagrams per major topic.
...
TIMELINE for historical events — STRICT RULES:
- NEVER use colons inside event text
- Replace ratios like 60:40 with "60-40"
...
COMPARISON for two or more entities:
\`\`\`mermaid
graph TD
  %% Comparison Title
  subgraph "Option A"
...
- NEVER use <br/> tags in labels — use space
- NEVER use () {} in labels
...
List the 10 most important points.

## ❓ Likely Exam Questions
5 important questions with brief answers.

OUTPUT: Pure Markdown only. Start with 📐 line. Nothing before it.`;
}
